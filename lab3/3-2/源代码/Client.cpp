...
random_device rd;
mt19937 gen(rd());
uniform_int_distribution<> dis(20, 32);
int randomInt = dis(gen);

int cwnd = 1; // 初始拥塞窗口大小 (以报文为单位)
int ssthresh = 64; // 慢启动阈值 (以报文为单位)
int dupACKcount = 0; // 连续重复 ACK 的计数
enum CongestionState { SlowStart, CongestionAvoidance, FastRecovery };
...
DWORD WINAPI ACKRecvThread(PVOID pParam) {
	...
	while (1) {
		...
		//这里加的是一个全局互斥锁，保护共享资源(如输出cout和滑动窗口变量base，避免多线程环境下的数据竞争)
		if (recvByte > 0) {  // 成功收到消息【不管是啥，收到东西了】
			if ((recvMsg.Flag && ACK) && recvMsg.check()) {  // 检查校验和
				...
				if (recvMsg.ack < base);// 如果收到的 ACK 小于 base，则忽略（失序 ACK）
                else if (recvMsg.ack == base){
                        dupACKcount++;
                        if (dupACKcount == 3) {
                        // 进入快速恢复
                        ssthresh = max(cwnd / 2, 1); // 更新慢启动阈值
                        cwnd = ssthresh + 3; // 临时增加 cwnd
                        congestionState = FastRecovery;
                        // 重传丢失的分组
                        MsgFormat retransmitMsg = sendtmp;
                        sendto(clientSocket, (char*)&retransmitMsg, sizeof(retransmitMsg), 0, (sockaddr*)&serverAddr, addrlen);
                        cout << "[传输日志] 快速重传 seq = " << retransmitMsg.seq << endl;
                    }
                }
				else
					base = recvMsg.ack + 1;  // 滑动窗口滑动
                    dupACKcount = 0; // 重置重复 ACK 计数
                    if (base != nextseqnum)
...
int main() {
	...
	cout << endl;
	cout << "===============================================" << endl;
	cout << "          佛祖保佑    iii    永不死机" << endl;
	cout << "          心外无法           法外无心" << endl;
	cout << "               佛法传输客户端v2.0 " << endl;
	cout << "                    By KKkai" << endl;
	cout << "===============================================" << endl;
	cout << "==================客户端准备===================" << endl;
	cout << "================当前滑动窗口大小:" << N << "===========" << endl;
	//========================初始化Socket DLL=========================
